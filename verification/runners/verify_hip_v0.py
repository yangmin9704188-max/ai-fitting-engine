#!/usr/bin/env python3
"""
HIP v0 Verification Runner

Purpose: Record validation results for measure_hip_v0.
...
try:
    ...
    print("WARNING: core.measurements.hip_v0 not found. Using stub.")
    ...
    def measure_hip_v0(verts, measurement_key="HIP", units_metadata=None):
        """Stub function for testing."""
        raise NotImplementedError("measure_hip_v0 not yet implemented")
...
def verify_case(
    ...
    """
    Verify a single case for HIP measurement.
    ...
    try:
        ...
        output: HipResult = measure_hip_v0(
            verts=verts,
            measurement_key="HIP",
            units_metadata=None,
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Verify HIP v0 (factual recording only)"
    )
    parser.add_argument(
        "--npz",
        type=str,
        default="verification/datasets/golden/hip_v0/s0_synthetic_cases.npz",
        help="Path to NPZ file containing verts and case_id",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        default="verification/reports/hip_v0",
