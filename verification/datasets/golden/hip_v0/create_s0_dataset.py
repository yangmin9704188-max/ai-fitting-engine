#!/usr/bin/env python3
"""Create s0_synthetic_cases.npz dataset for hip v0 validation."""
...
# Case 7: Upper quantile ambiguous (multiple similar max values)
# Create shape with multiple peaks near upper quantile region
n_verts = 100
verts = np.zeros((n_verts, 3), dtype=np.float32)
for j in range(n_verts):
    x = (j % 10) / 10.0 * 0.5 - 0.25
    y = (j // 10) / 10.0 * 1.0
    z = ((j % 5) / 5.0) * 0.3 - 0.15
    # Add multiple similar peaks in upper region (HIP region)
    if 0.5 <= y <= 0.8:  # Upper torso region
        z += np.sin(j * 0.5) * 0.05  # Create similar peaks
    verts[j] = [x, y, z]
cases.append(verts)
case_ids.append("upper_quantile_ambiguous")
