---
title: "Validation Frame: BUST/UNDERBUST v0"
version: "v0.1"
status: "draft"
...
# Validation Frame: BUST/UNDERBUST v0
...
본 계약은 `core/measurements/bust_v0.py::measure_bust_v0` 및 `core/measurements/underbust_v0.py::measure_underbust_v0`에 대한 검증 프로토콜을 정의합니다.
...
- 형식: NPZ 파일 (`verification/datasets/golden/bust_underbust_v0/s0_synthetic_cases.npz`)
- 포함 케이스:
  - 정상 케이스 (박스/원통 유사) 2개 (UNDERBUST용 1개, BUST용 1개)
...
- Type: `Literal["UNDERBUST"]` 또는 `Literal["BUST"]`
...
**File**: `verification/runners/verify_bust_underbust_v0.py`

**Execution**:
```bash
python verification/runners/verify_bust_underbust_v0.py [--npz <path>] [--out_dir <path>]
```

**Process**:
1. Load NPZ dataset
2. For each case_id:
   - Call `measure_underbust_v0(verts, measurement_key="UNDERBUST")` (UNDERBUST 케이스)
   - Call `measure_bust_v0(verts, measurement_key="BUST")` (BUST 케이스)
   - Record output to CSV
   - Handle exceptions (record failure_type)
   - Check determinism (2회 호출 비교)
3. Generate summary JSON

⚠️ TBD: 실제 runner 구현은 구현 단계에서 수행됨
...
**File**: `verification/reports/bust_underbust_v0/validation_results.csv`

**Columns**:
- `case_id`: str
- `measurement_key`: str ("UNDERBUST" 또는 "BUST")
...
**File**: `verification/reports/bust_underbust_v0/validation_summary.json`
...
- `large_delta_count`: int (LARGE_BUST_UNDERBUST_DELTA warning 발생 횟수, 사실 기록만)
...
- `verts.shape[1] != 3`
- `verts.ndim != 2`
- `measurement_key`가 "UNDERBUST" 또는 "BUST"가 아님
...
1. **Dataset**: `verification/datasets/golden/bust_underbust_v0/s0_synthetic_cases.npz`
2. **Runner**: `verification/runners/verify_bust_underbust_v0.py`
3. **Test**: `tests/test_bust_underbust_v0_validation_contract.py`
4. **Output CSV**: `verification/reports/bust_underbust_v0/validation_results.csv`
5. **Output JSON**: `verification/reports/bust_underbust_v0/validation_summary.json`

⚠️ TBD: 실제 파일 생성은 구현 단계에서 수행됨
...
- `measurement_key`: `(N_cases,)` array of str ("UNDERBUST" or "BUST")
...
- No action directives (Judgment layer excluded)
