...
def generate_triggers_json_for_test():
    """Generate triggers.json with AUDIT_FAILED=true for test/** push."""
...
def main():
    ...
    # Priority 2: test/** push - force AUDIT_FAILED for smoke test
    elif event_name == 'push' and ref_name.startswith('test/'):
        print(f"Event: {event_name} | Branch: {ref_name} | Action: Force AUDIT_FAILED for smoke test")
        triggers = generate_triggers_json_for_test()
    # Priority 3: existing triggers.json (manual test file)
        ...
        triggers = normalized
    # Priority 4: extract from evidence files (normal PR flow)
    else:
